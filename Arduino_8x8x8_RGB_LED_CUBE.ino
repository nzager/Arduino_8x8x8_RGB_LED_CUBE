...
Layer select conversion for SuperTech-IT Printed Circuit board
by Norman Skinner - August 2014
Also porting over to Arduino MEGA2560
www.TheLEDCube.com
...
Pin definition clarification (SuperTech-IT)
Layer 8 - Uno pin - A5 is the same ad D19 - but in an array, we must use 19 to specify A5.
...
void test() {
    ...
    LED(xxx, yyy, zzz, 15, 15, 15);
    }}}
//    for(int xxx=0; xxx<8; xxx++){
//    for(int yyy=0; yyy<8; yyy++){
//    for(int zzz=0; zzz<8; zzz++){
//    //LED(0, yyy, 1, 10, 10, 10);
//    }}}
//    delay(5000);
}
...
void displayScrollingLetter(char c, int R, int G, int B) {
  ...
  for (int i = 0; i < 8; i++) {
    display[i] = 0x00;
    beforePanel[i] = font_data[c][j] << 1;
    j--;
  }
  for (int scroll = 0; scroll < 14; scroll++) {
    for (int level = 0; level < 8; level++) {
        int row = 0;
        for (unsigned int mask = 0x80; mask != 0; mask >>= 1) {
            if (display[level] & mask) {
      ...
      delay(150);
        ...
        for (int i = 0; i < 8; i++) {
          ...
          if (beforePanel[i] & mask) {
            display[i]++;
          }
          beforePanel[i] = beforePanel[i] << 1;
        }
